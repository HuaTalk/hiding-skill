...
Automatic selection resolves output artifacts in priority order: explicit literal paths are included; known agent/tool control state is excluded; requested task deliverables are included; then human/project-consumed files are included while agent-only files are excluded. If the role of a file is still unclear, `/hiding` preserves it and asks before scanning it. Under `--dry-run`, it lists unresolved candidates without scanning them.
